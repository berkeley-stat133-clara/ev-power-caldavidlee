...
```{r}
str_detect(energy_sources, "Wind") | str_detect(energy_sources, "Hydro")
```

b.  Extract the unit type (e.g., MWh, GWh, kWh) using str_extract(). Make sure your pattern handles extra spaces or hyphens.

```{r}
#str_extract(energy_sources, "(?<=\\()[^)]*(?=\\))") WORKING SOLUTION CHATGPT
str_extract(energy_sources, "\\(...\\)")
```
...
```{r}
inner_join(x=states, y = regions, by = join_by(state == state))
```
...
```{r}
anti_join(x= states, y= regions, by = join_by(state == state))
```

d.  Discussion: Why would you prefer a left join over an inner join when merging state-level datasets?\
    \
    I would prefer a left_join as it will add columns from DF2 to DF1 and all states are matching so will be a full join essentially.
e.  Use pivot_longer() to make the socioeconomic data longer.\

```{r}
states |>
pivot_wider(names_from = Illiteracy, values_from  = state)
```
...
```{r}
us_map_joined <- right_join(x=us_map, y = state_data, by = join_by(region == state))
us_map_joined
```
